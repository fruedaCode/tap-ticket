...
export function LandingPage() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col bg-background">
      {/* Overlays the hero instead of sitting above it, so the hero is a full
          screen rather than one screen minus the header. */}
      ...
      <main className="flex flex-1 flex-col">
        ...
        <div className="mx-auto flex w-full max-w-4xl flex-col px-4">
          ...
          {/* Visible FAQ doubles as the FAQPage JSON-LD source (lib/seo/faq):
              long-tail questions are where "scan ticket with ai" searches land. */}
          <section className="pb-16 sm:pb-24">
            <h2 className="mb-8 text-center text-2xl font-semibold tracking-tight">
            ...
            <dl className="flex flex-col gap-6">
              {FAQ_ITEMS.map((item, i) => (
                <motion.div key={item.q} {...reveal(i)}>
                  <dt className="font-semibold text-balance">{t(item.q)}</dt>
                  <dd className="mt-1 text-sm text-muted-foreground">{t(item.a)}</dd>
                </motion.div>
              ))}
            </dl>
          </section>
