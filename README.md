...
Built as a **Next.js 16 PWA** backed by **Supabase** (Postgres + Auth + Realtime + Storage), with receipt OCR via **Mistral Document AI** — EU-hosted, so receipt images never leave the EU. Three other scanners ship behind `AI_PROVIDER`; see [Receipt scanning providers](#receipt-scanning-providers).

## Prerequisites

- Node.js 22
- A [Supabase](https://supabase.com) project
- A [Mistral](https://console.mistral.ai) API key, or a key for one of the other providers (optional — see `MOCK_SCAN` below)
...
| `AI_PROVIDER` | Which scanner to use: `mistral`, `scaleway`, `ovh` (all EU) or `groq` (US). Defaults to `mistral`. See [Receipt scanning providers](#receipt-scanning-providers) |
...
| `MOCK_SCAN` | Set to `true` to fake scan results in dev — no provider key needed |
...
2. Set the server-only secrets:
   ...
   ```bash
   fly secrets set MISTRAL_API_KEY=… SUPABASE_SERVICE_ROLE_KEY=…
   ```
...
receipts. `mistral` is the default and what production runs. `groq` is the original, US-hosted one,
kept for comparison — selecting it reintroduces an international transfer that `/legal/privacy` §5
says does not happen, so it would need the policy changed back.
...
- **AI transparency** — the scan dialog says AI (Mistral AI) will read the receipt and may be wrong,
  and the ticket screen tells users the items were AI-extracted and are editable (human oversight).
  **If you change `AI_PROVIDER`, update this disclosure and `/legal/privacy` §4 to match** — naming a
  processor you don't use is an Art. 13 problem in its own right.
- **No international transfers** — every processor is in the EU, so `/legal/privacy` §5 states that
  plainly and no Chapter V safeguard is needed.
...
| --- | --- | --- |
| **Mistral AI** (EU) | Accept the Data Processing Addendum published at `mistral.ai/terms`, and keep their subprocessor list with your art. 30 record. Their docs state "data sent through the API isn't used for model training", which is worth keeping a copy of. No Chapter V safeguard needed — processing stays in the EU. | Self-serve accept |
